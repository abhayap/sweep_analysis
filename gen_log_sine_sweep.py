...
def main() :
    ...
    Nsamp = np.ceil(opts.rate*opts.duration) # number of samples in sweep
    ...
    if opts.fadein :
        ...
        ramp = np.power(np.sin(np.linspace(0, np.pi/2.0, opts.rate*opts.fadein)),2.0)
    ...
    if opts.silence :
        ...
        silence = np.zeros(opts.rate*opts.silence)
